<?php error_reporting(E_ERROR | E_PARSE);//to report about runtime errors
include 'func.php'; // including the func.php file in to this file

if (isset($_GET['do']) && $_GET['do'] == 'compare') { //checks if do = compare and not null
	$eg = false;
	$txtAr[1] = html_entity_decode($_POST['cont1']); //Convert HTML entities to their corresponding characters file-1
	$txtAr[2] = html_entity_decode($_POST['cont2']);// file-2
	$compareAr = array(
		array(1, 2)
	); 
...
<div class="examples">
	...
	<div class="col-lg-10 no-padding">
		...
		<?php for ($i = 1; $i <= 7; $i++) { ?>
			<div class="col-lg-4 no-padding-left code-content-<?php echo $i ?>">
				<h3 class="ppd" id="p<?php echo $i ?>"><i>p<?php echo $i ?></i></h3>
				<pre class="code" style="height:170px"><?php  echo $txtAr[$i] ?></pre>
			</div>
		<?php } ?>
	</div>
	<div class="clearfix"></div>
